...
STREET_REGEXP = re.compile(
    r"(?P<street>(?:ул\.|улица|пер\.|переулок|аллея|бульвар|вал|взвод|въезд|дорога|заезд|кольцо|линия|луч|магистраль|"
    r"набережная|перспектива|пр-кт|площадь|проезд|проспект|проулок|разъезд|спуск|съезд|тракт|тупик|шоссе)"
    r"\s[\wА-Яа-я\-\ \.]+[^д|д\.|дом])[\.\,]?\s(?P<house>(?:д\.?|дом)?\s?[\d\/\\\-а-яa-z]+)$"
)


class RKeeperSettings(Base):
    id: str
    client_id: str = Field(
        ...,
        description="Выданный для [подключения по API](https://docs.rkeeper.ru/"
        "delivery/administrativnaya-panel/integratsii/podklyuchenie-po-api)",
    )
...
class RKeeperShop(Base):
    ...
    def convert_to_pos_creator(self) -> CreateShop:
        street_match = STREET_REGEXP.search(self.actual_address) if self.actual_address else None
        address = Address(
            latitude=self.actual_address_lat,
            longitude=self.actual_address_lon,
            street=street_match["street"] if street_match else self.actual_address,
            house=street_match["house"] if street_match else None,
            city=self.city,
            comment=self.actual_address,
        )
